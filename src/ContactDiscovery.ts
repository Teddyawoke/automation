  ...
  start: function() {
    ...
    try {
      ...
      if (candidates.length === 0) {
        statusCell?.setValue('✅ All Qualified Companies Processed');
        return;
      }

      // 2. Send to Robot
      statusCell?.setValue(`🤖 Sending ${candidates.length} domains to Robot...`);
      ...
      statusCell?.setValue('⏳ Robot Running (Check back later)...');
